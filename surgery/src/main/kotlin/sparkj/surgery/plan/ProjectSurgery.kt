...
interface ProjectSurgery {
    ...
    // 1 现在处理 --> transform
    // 2 以后处理 --> 收集起来
    fun surgeryOnFile(srcFile: File, srcDirectory: File, destDirectory: File, status: ospl.sparkj.surgery.api.Status)

    //过滤文件
    // 1 现在处理 --> 扫描jar-> transform
    // 2 以后处理 --> 扫描jar-> 收集起来
    fun surgeryOnJar(srcJarFile: File, destJarFile: File, status: ospl.sparkj.surgery.api.Status)
...
class ProjectSurgeryImpl : ProjectSurgery {
    ...
    override fun surgeryOnFile(srcFile: File, srcDirectory: File, destDirectory: File, status: ospl.sparkj.surgery.api.Status) {
        scheduler.submit {
            if (status == ospl.sparkj.surgery.api.Status.NOTCHANGED) {
            ...
            val nowGroup = grouped[FilterAction.transformNow] ?: emptyList<ClassBytesSurgery>() + nowLastGroup
    ...
    override fun surgeryOnJar(srcJarFile: File, destJarFile: File, status: ospl.sparkj.surgery.api.Status) {
        //处理jar的时候
        // 对于jar里面的class
