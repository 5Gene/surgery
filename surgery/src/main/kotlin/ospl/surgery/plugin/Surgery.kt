    ...
    private fun reviewJarFile(
        ...
        it.jarInputs.onEach { jar ->
            ...
            if (transformInvocation.isIncremental) {
                ...
                when (jar.status!!) {
                    Status.NOTCHANGED -> {
                        surgery.surgeryOnJar(jar.file, destJarFile, ospl.surgery.api.Status.NOTCHANGED)
                    }
                    Status.ADDED, Status.CHANGED -> {
                        surgery.surgeryOnJar(jar.file, destJarFile, ospl.surgery.api.Status.ADDED)
                    }
                    Status.REMOVED -> {
        ...
        it.directoryInputs.onEach { dir ->
            ...
            if (transformInvocation.isIncremental) {
                ...
                dir.changedFiles.onEach { entry ->
                    ...
                    when (entry.value!!) {
                        Status.NOTCHANGED -> {
                            ...
                            surgery.surgeryOnFile(entry.key, srcDirectory, destDirectory, ospl.surgery.api.Status.NOTCHANGED)
                        }
                        Status.ADDED, Status.CHANGED -> {
                            surgery.surgeryOnFile(entry.key, srcDirectory, destDirectory, ospl.surgery.api.Status.ADDED)
                        }
                        Status.REMOVED -> {
                ...
                srcDirectory.walk().filter { it.isFile }.forEach {
                    surgery.surgeryOnFile(it, srcDirectory, destDirectory, ospl.surgery.api.Status.ADDED)
                }
            }
        }
    }
